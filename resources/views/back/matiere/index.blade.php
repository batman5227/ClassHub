                    ...
                    <div class="col-lg-8">
                        <nav aria-label="breadcrumb" class="mb-3">
                            <ol class="breadcrumb">
                                ...
                                <li class="breadcrumb-item active text-blue" aria-current="page">Matières</li>
                            </ol>
                        </nav>
                        <h1 class="display-4 fw-bold text-blue mb-3">Gestion des matières</h1>
                        <p class="text-blue opacity-90 lead mb-4">Gérez les matières de l'établissement</p>
                        <div class="d-flex gap-3">
                            <div class="bg-blue bg-opacity-20 rounded-3 px-4 py-2">
                                <small class="text-blue opacity-75 d-block">Total matières</small>
                                <span class="text-blue fw-bold">{{ $matieres->total() }}</span>
                            </div>
                        </div>
                    </div>
                    <div class="col-lg-4 text-lg-end mt-4 mt-lg-0">
                        <a href="{{ route('matieres.create') }}"
                           class="btn btn-light btn-lg rounded-pill px-5 shadow-sm hover-lift">
                            <i class="fas fa-plus-circle me-2 text-primary"></i>Nouvelle matière
                        </a>
                    </div>
                </div>
            </div>
    ...
    <div class="card border-0 shadow-lg rounded-4 overflow-hidden">
            ...
            <div class="row align-items-center">
                <div class="col">
                    <h4 class="mb-0 fw-bold">
                        <i class="fas fa-list text-primary me-2"></i>Liste des matières
                    </h4>
                    <p class="text-muted mb-0 mt-1">{{ $matieres->total() }} matière(s) trouvée(s)</p>
                </div>
                <div class="col-auto">
                    <div class="input-group">
                        ...
                        <input type="text" class="form-control border-start-0" id="search-matieres"
            ...
            <div class="table-responsive">
                <table class="table table-hover align-middle mb-0">
                    <thead class="bg-light">
                        <tr>
                            <th class="px-4 py-3">Nom de la matière</th>
                    ...
                    <tbody>
                        @forelse ($matieres as $matiere)
                            <tr class="matiere-row" data-name="{{ strtolower($matiere->nom . ' ' . ($matiere->classe->nom ?? '')) }}">
                                <td class="px-4 py-3">
                                    <div class="d-flex align-items-center">
                                        ...
                                        <div>
                                            ...
                                            <small class="text-muted d-block">
                                                ID: {{ substr($matiere->id, 0, 6) }}...{{ substr($matiere->id, -4) }}
                                            </small>
                                        </div>
                                    </div>
                                </td>
                                <td class="px-4 py-3">
                                    @if($matiere->classe)
                                        <span class="badge bg-info bg-opacity-10 text-info rounded-pill px-3 py-2">
                                            <i class="fas fa-users me-1"></i>{{ $matiere->classe->nom }}
                                        </span>
                                    @else
        ...
        <div class="card-footer bg-white border-0 p-4">
            <div class="d-flex justify-content-between align-items-center">
                <span class="text-muted">Affichage de {{ $matieres->count() }} matière(s)</span>
...
(function () {
    ...
    function init() {
        ...
        // Recherche
        var searchInput = document.getElementById('search-matieres');
        if (searchInput) {
