                    ...
                    <div class="col-lg-4 text-lg-end mt-4 mt-lg-0">
                        <a href="{{ route('documents.create') }}"
                           class="btn btn-light btn-lg rounded-pill px-5 shadow-sm hover-lift">
                            <i class="fas fa-plus-circle me-2 text-primary"></i>Nouveau document
                        </a>
                    </div>
                </div>
            </div>
    ...
    <div class="card border-0 shadow-lg rounded-4 overflow-hidden">
            ...
            <div class="row align-items-center">
                <div class="col">
                    <h4 class="mb-0 fw-bold">
                        <i class="fas fa-list text-primary me-2"></i>Liste des documents
                    </h4>
                    <p class="text-muted mb-0 mt-1">{{ $documents->total() }} document(s) trouvé(s)</p>
                </div>
                <div class="col-auto">
                    <div class="input-group">
                        ...
                        <input type="text" class="form-control border-start-0" id="search-documents"
        ...
        <div class="card-body p-0">
            @if($documents->isEmpty())
                <div class="text-center py-5">
                    ...
                    <p class="text-muted mb-3">Commencez par ajouter votre premier document</p>
                    <a href="{{ route('documents.create') }}"
                                ...
                                <tr class="document-row" data-name="{{ strtolower($document->nom . ' ' . ($document->matiere->nom ?? '')) }}">
                                    <td class="px-4 py-3">
                                        <div class="d-flex align-items-center">
                                            ...
                                            <div>
                                                <span class="fw-semibold">{{ $document->nom }}</span>
                                                <small class="text-muted d-block">
                                                    ID: {{ substr($document->id, 0, 6) }}...{{ substr($document->id, -4) }}
                                                </small>
                                            </div>
                                        </div>
                                    </td>
                                    <td class="px-4 py-3">
                                        ...
                                        @else
                                            <span class="badge bg-secondary bg-opacity-10 text-secondary rounded-pill px-3 py-2">
                                                <i class="fas fa-times-circle me-1"></i>Non associé
                                            </span>
                                        @endif
                                    </td>
                                    <td class="px-4 py-3">
                                    ...
                                    <td class="px-4 py-3">
                                        @if($document->fichier)
                                            @php
                                                ...
                                                $tailleFormatee = $taille > 0 ? round($taille / 1024, 2) . ' KB' : 'Inconnue';
                                            @endphp
                                            <span class="badge bg-success bg-opacity-10 text-success rounded-pill px-3 py-2">
                                                <i class="fas fa-database me-1"></i>{{ $tailleFormatee }}
                                            </span>
                                        @else
                                            <span class="badge bg-danger bg-opacity-10 text-danger rounded-pill px-3 py-2">
                                                <i class="fas fa-times-circle me-1"></i>Fichier manquant
                                            </span>
                                        @endif
                                    </td>
        ...
        <div class="card-footer bg-white border-0 p-4">
            <div class="d-flex justify-content-between align-items-center">
                <span class="text-muted">Affichage de {{ $documents->count() }} document(s)</span>
